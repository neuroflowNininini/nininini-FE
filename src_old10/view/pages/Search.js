...
import e1 from "../../shared/pics/nail/E1.jpg"
...
export default function Search({ closeModal }) {
  ...
  return (
    <Background>
      <IconWrap>
        <VscChromeClose onClick={closeModal} size="30" color="black" />
      </IconWrap>
      <TopWrap id="hometop" >
        <Space />
        <Hang>
          <Input value={keyword} // 입력 필드의 값은 상태 변수와 바인딩됩니다.
            placeholder='  검색어를 입력하세요'
            onChange={handleKeywordChange} // 입력 필드 값이 변경될 때 핸들러 호출           
          />
          <PiMagnifyingGlass onClick={handleSearch} size="30" color="black" />
        </Hang>
        <Hang1>
          <Label>#가을네일</Label>
          <Label>#마블네일</Label>
